...
def login_google(browser):
    conf = init_json_config()
    email = conf[0]['user'][0]['email']
    password = conf[0]['user'][0]['password']
...
def login_meet(browser, classroom):
    conf = init_json_config()
    for i in conf[1]['meets']:
        print(i)
        if classroom == i['name']:
            ...
            browser.find_element_by_xpath(code_xpath).send_keys(i['code'])
            ...
            time.sleep(3)
            mic_xpath="/html/body/div[1]/c-wiz/div/div/div[8]/div[3]/div/div/div[2]/div/div[1]/div[1]/div[1]/div/div[4]/div[1]/div/div/div"
